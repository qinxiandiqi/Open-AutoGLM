...
| target_app_name | string | null | 目标应用显示名称（如"今日头条"） |
| target_app_package | string | null | 目标应用包名（如"com.ss.android.article.news"） |
...
auto_patrol:
  enabled: true
  target_app_name: "今日头条"        # 应用显示名称（用于日志、提示等）
  target_app_package: "com.ss.android.article.news"  # 应用包名（用于启动、关闭等操作）
  max_pages: 15
  max_depth: 2
  max_time: 180
  forbidden_actions:
...
auto_patrol:
  enabled: true
  target_app: "今日头条"  # 应用名（会自动查找包名）
  # 或
  target_app: "com.ss.android.article.news"  # 包名（会自动反查应用名）
```

**说明**:
- **新格式（推荐）**：分别指定 `target_app_name` 和 `target_app_package`，配置更清晰明确
- **旧格式（兼容）**：使用 `target_app`，系统会根据值自动判断是应用名还是包名
- **应用名称**：如"今日头条"、"微信"等（用于日志显示和用户提示）
- **包名**：如 `com.ss.android.article.news`（用于实际的操作：启动、关闭应用）
- 两种格式可以混用，新格式优先级更高
...
auto_patrol:
  enabled: true
  target_app_name: "今日头条"
  target_app_package: "com.ss.android.article.news"
  max_pages: 15
  max_depth: 2
  max_time: 180
  forbidden_actions:
